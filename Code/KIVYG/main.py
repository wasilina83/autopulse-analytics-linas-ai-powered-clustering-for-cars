...
class SignalclassifierApp(App):
    ...
    def on_play_button_click(self, instance):
        ...
        self.amplitude_slider = CustomSlider(min=3, max=5, step=.25, orientation='horizontal', size_hint=(None, None), size=(int(window_width*.33), int(window_height*.05)), pos_hint={'center_x': 0.2, 'top': 0.83})
        self.frequency_slider = CustomSlider(min=159, max=165, step=1, orientation='horizontal', size_hint=(None, None), size=(int(window_width*.33), int(window_height*.05)), pos_hint={'center_x': 0.2, 'top': 0.7})
