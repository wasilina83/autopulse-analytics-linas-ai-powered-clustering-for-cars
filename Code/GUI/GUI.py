...
class SignalclassifierApp(App):
    ...
    def on_play_button_click(self, instance):
        ...
        self.frequency_slider = Slider(min=1, max=10, step=1, orientation='horizontal', size_hint=(None, None), size=(500, 200), pos_hint={'center_x': 0.2, 'top': 0.7}, value_track=True, value_track_color='blue')
        self.offset_slider = Slider(min=-1, max=1, step=.1, orientation='horizontal', size_hint=(None, None), size=(500, 200), pos_hint={'center_x': 0.2, 'top': 0.6}, value_track_color= (0, 0.2745, 0.9412, 1), value_track=True)
    ...
    def on_play_button_click2(self, instance):
        ...
        if sigTest == 0:
            self.sigpicture = Image(source='GUI/images/test-1.png', size_hint=(None, None),  allow_stretch=True, size=(800, 800), pos_hint={'center_x': .69, 'top': 0.9})
